...
for file in os.listdir(Calculated_Tt_path):
    ...
    calc_Tt = calc_Tt[['Date', 'Total Degree Days', 'Sum Unaffected Daily Thermal Time']]
    ...
    affected_thermal_data.update({file[:file.rfind('.')]:calc_Tt[['Date','Growing Year','Day_of_Growing_Season','Total Degree Days']]})
...
for plant_ID in unaffected_thermal_data:
    for year, group in unaffected_thermal_data[plant_ID].groupby('Growing Year'):
        plt.plot(group['Day_of_Growing_Season'], group['Sum Unaffected Daily Thermal Time'], linestyle=':', label=f'Unaffected {plant_ID}')
for plant_ID in affected_thermal_data:
    for year, group in affected_thermal_data[plant_ID].groupby('Growing Year'):
        plt.plot(group['Day_of_Growing_Season'], group['Total Degree Days'], linestyle='--', label=f'Affected {plant_ID}')
